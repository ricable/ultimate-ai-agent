...
async function demonstrateReasoningBankIntegration() {
  ...
  const config: ReasoningBankConfig = {
    agentDB: {
      ...
      syncProtocol: 'QUIC',
      persistenceEnabled: false, // Disabled for demo
  ...
  const standardDeviation = Math.sqrt(
    benchmarkTimes.reduce((sum, time) => sum + Math.pow(time - averageTime, 2), 0) / benchmarkIterations
  );
...
export { demonstrateReasoningBankIntegration };