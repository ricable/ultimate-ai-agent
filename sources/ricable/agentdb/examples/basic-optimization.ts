...
import { RANOptimizationSDK, DEFAULT_CONFIG } from '../src/sdk/ran-optimization-sdk';

/**
 * Basic RAN metrics for optimization
 */
const basicRANMetrics = {
  energy_efficiency: 0.75,        // 75% efficient
  mobility_performance: 0.80,     // 80% optimal
  coverage_quality: 0.85,         // 85% coverage
  capacity_utilization: 0.70,     // 70% utilized
  user_experience: 0.78,          // 78% satisfaction
...
async function demonstrateSkillDiscovery() {
  ...
  try {
    ...
    if (relevantSkills.length > 0) {
      ...
      try {
        const resources = await skillDiscovery.loadSkillResource(
          firstSkill.name,
