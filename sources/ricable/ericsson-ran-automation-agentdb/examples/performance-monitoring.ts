...
async function example1_QuickStart() {
  ...
  try {
    ...
    console.log('\n🎯 Performance Targets:');
    Object.entries(overview.performanceTargets).forEach(([target, data]: [string, any]) => {
  ...
  try {
    ...
    if (agentdbHealth) {
      ...
      if (agentdbHealth.recommendations && agentdbHealth.recommendations.length > 0) {
        ...
        agentdbHealth.recommendations.forEach((rec: any, index: number) => {
    ...
    if (cognitive && cognitive.summary) {
      ...
      if (cognitive.predictions && cognitive.predictions.length > 0) {
        ...
        cognitive.predictions.forEach((pred: any) => {
...
export {
  example1_QuickStart,
  example2_AdvancedMonitoring,
  example3_PerformanceAnalysis,
  example4_RealTimeDashboard
};