...
try:
    ...
    MultimodalDatasetGenerator = None
    MultimodalDatasetConfig = None
...
class DocumentPipeline:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None):
        ...
        self.augmentoolkit_generator = None
        self.mlx_finetuner = None
        self.multimodal_generator = None
        
        if HAS_AUGMENTOOLKIT:
            try:
                augmentoolkit_config = AugmentoolkitConfig()
                self.augmentoolkit_generator = AugmentoolkitGenerator(augmentoolkit_config)
            except Exception as e:
                logger.warning(f"Failed to initialize AugmentoolkitGenerator: {e}")
        
        if HAS_MLX:
            try:
                self.mlx_finetuner = MLXFineTuner()
            except Exception as e:
                logger.warning(f"Failed to initialize MLXFineTuner: {e}")
        
        if HAS_MULTIMODAL:
            try:
                multimodal_config = MultimodalDatasetConfig(
                    domain_expertise="telecommunications and 5G networks",
                    dataset_context="technical documentation with tables and diagrams"
                )
                self.multimodal_generator = MultimodalDatasetGenerator(multimodal_config)
            except Exception as e:
                logger.warning(f"Failed to initialize MultimodalDatasetGenerator: {e}")
    ...
    def concatenate_markdown_files(self, markdown_files: List[str], output_path: str) -> str:
        ...
        """
        print(f"[COMBINE] Concatenating {len(markdown_files)} Markdown files without metadata")
    ...
    def create_rag_datasets(self, chunks: List, output_dir: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Generate multimodal datasets if enabled and available
            if self.config.docling.multimodal and self.multimodal_generator:
                try:
                    logger.info("🎨 Generating multimodal vision-language datasets...")
                    multimodal_results = self.multimodal_generator.generate_multimodal_dataset(
                        chunks, 
                        os.path.join(output_dir, "multimodal"),
                        "multimodal_dataset"
                    )
                    logger.info(f"✅ Multimodal datasets created: {list(multimodal_results.get('datasets', {}).keys())}")
                except Exception as e:
                    logger.error(f"❌ Failed to generate multimodal datasets: {e}")
                    self.stats["errors"].append(f"Multimodal dataset generation error: {str(e)}")
    ...
    def create_enhanced_finetune_datasets(self, chunks: List, output_dir: str) -> Dict[str, str]:
        ...
        # 2. Conversation format (Chat/Assistant training)
        conversation_data = self._create_conversation_dataset(chunks)
        conversation_path = os.path.join(output_dir, "finetune_conversation.json")
        with open(conversation_path, 'w', encoding='utf-8') as f:
            json.dump(conversation_data, f, indent=2, ensure_ascii=False)
        results["conversation"] = conversation_path
        
        # 3. JSONL format for streaming training
        ...
        train_data, val_data = self._create_train_val_split(chunks)
        
        train_path = os.path.join(output_dir, "finetune_train.json")
        with open(train_path, 'w', encoding='utf-8') as f:
            json.dump(train_data, f, indent=2, ensure_ascii=False)
        results["train"] = train_path
        
        val_path = os.path.join(output_dir, "finetune_validation.json")
        with open(val_path, 'w', encoding='utf-8') as f:
            json.dump(val_data, f, indent=2, ensure_ascii=False)
        results["validation"] = val_path
        
        # 5. Legacy simple format (for compatibility)
        simple_data = {
            "data": [
                {
                    "id": chunk.chunk_id,
    ...
    def _create_instruction_response_dataset(self, chunks: List) -> Dict[str, Any]:
        ...
        import random
        training_data = []
        seen_combinations = set()  # Track instruction-content combinations
        
        # Diverse instruction templates
        instruction_templates = [
            "Explain the content from {source} about the following topic.",
            "Provide information about what is described in this section of {source}.",
            "Summarize the key points from this part of the {source} documentation.",
            "What does this section from {source} cover?",
            "Describe the information contained in this excerpt from {source}.",
            "What are the main concepts discussed in this {source} section?",
            "How does this part of {source} relate to system configuration?",
            "What technical details are provided in this {source} excerpt?",
            "Explain the implementation described in this {source} section.",
            "What are the key features mentioned in this {source} content?"
        ]
        ...
        for chunk in chunks:
            ...
            # Create 1-2 high-quality examples per chunk instead of 5
            selected_templates = random.sample(instruction_templates, min(2, len(instruction_templates)))
            
            for template in selected_templates:
                instruction = template.format(source=chunk.source)
                
                # Create a unique key to avoid duplicates
                content_hash = hash(chunk.text[:100])  # Use first 100 chars for similarity
                combination_key = (instruction, content_hash)
                
                if combination_key in seen_combinations:
                    continue  # Skip duplicate instruction-content combinations
                    
                seen_combinations.add(combination_key)
                
                # Intelligent input/output creation to avoid data leakage
                if len(chunk.text) > 200:
                    # Use a meaningful excerpt as input, full text as output
                    input_text = self._create_meaningful_input(chunk.text)
                    output_text = self._create_summary_output(chunk.text)
                else:
                    # For short chunks, use title/topic as input
        ...
        return {
            ...
            "description": "Non-duplicate instruction-response pairs for LLM fine-tuning",
            "version": "2.0.0",
    ...
    def _create_summary_output(self, text: str) -> str:
        """Create summarized output instead of full text echo."""
        # For now, return cleaned full text
        # This could be enhanced with actual summarization
        return text.strip()
    ...
    def _create_conversation_dataset(self, chunks: List) -> Dict[str, Any]:
        """Create conversation format dataset.
        ...
        for chunk in chunks:
            # Generate conversational QA from chunk
            questions = [
                f"Can you tell me about {self._extract_topic(chunk.text)}?",
                f"What information is available about {self._extract_topic(chunk.text)}?",
                f"How does {self._extract_topic(chunk.text)} work?",
                f"Explain {self._extract_topic(chunk.text)} to me."
            ]
            
            for question in questions[:2]:  # Limit to 2 per chunk
                conversation = {
                    "messages": [
                        {
                            "role": "user",
        ...
        return {
            ...
            "format": "conversation",
            "description": "Conversational format for chat model fine-tuning",
            "version": "1.0.0"
        }
    ...
    def _create_finetune_jsonl(self, chunks: List) -> List[Dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            prompts = [
                ...
                f"Based on {chunk.source}, explain:"
            ]
    ...
    def _create_train_val_split(self, chunks: List, val_ratio: float = 0.2) -> Tuple[Dict, Dict]:
        ...
        """
        # Shuffle chunks for random split
        import random
        ...
        train_chunks = shuffled_chunks[val_size:]
        val_chunks = shuffled_chunks[:val_size]
        
        # Create datasets
        train_data = {
            "data": [{"id": chunk.chunk_id, "text": chunk.text, "source": chunk.source} 
                    for chunk in train_chunks],
            "size": len(train_chunks),
            "split": "train"
        }
    ...
    def validate_datasets(self, chunks: List, output_dir: str) -> Dict[str, Any]:
        ...
        duplicate_chunks = self._find_duplicate_chunks(chunks)
        
        report["validation_results"]["quality_checks"] = {
            "empty_chunks": len(empty_chunks),
            "duplicate_chunks": len(duplicate_chunks),
            "quality_score": (len(chunks) - len(empty_chunks) - len(duplicate_chunks)) / len(chunks) * 100 if chunks else 100
        }
    ...
    def _find_duplicate_chunks(self, chunks: List) -> List:
        """Find duplicate chunks in the dataset.
        ...
        for chunk in chunks:
            ...
            if text_hash in text_map:
                ...
                text_map[text_hash] = chunk.chunk_id
        
        return duplicates
    
    def _chunk_has_multimodal_content(self, chunk) -> bool:
        """Check if a chunk contains multimodal content.
        
        Args:
            chunk: Document chunk to analyze
            
        Returns:
            True if chunk contains tables, figures, or other multimodal content
        """
        import re
        ...
        if re.search(r'^\|.*\|\s*$', chunk.text, re.MULTILINE):
            return True
        if 'table' in text or 'csv' in text:
            return True
        
        # Check for figures
        if re.search(r'!\[.*?\]\(.*?\.(png|jpg|jpeg|gif|svg)\)', chunk.text, re.IGNORECASE):
            return True
        if 'figure' in text or 'image' in text or 'diagram' in text:
            return True
        
        return False
    
    def _get_chunk_multimodal_types(self, chunk) -> List[str]:
        """Get the types of multimodal content in a chunk.
        
        Args:
            chunk: Document chunk to analyze
            
        Returns:
            List of multimodal content types
        """
        import re
        types = []
        text = chunk.text.lower()
        ...
        # Check for tables
        if re.search(r'^\|.*\|\s*$', chunk.text, re.MULTILINE) or 'table' in text or 'csv' in text:
            types.append('table')
        
        # Check for figures
        if re.search(r'!\[.*?\]\(.*?\.(png|jpg|jpeg|gif|svg)\)', chunk.text, re.IGNORECASE) or 'figure' in text or 'image' in text or 'diagram' in text:
            types.append('figure')
        
        return types
    
    def _analyze_multimodal_chunks(self, chunks: List) -> Dict[str, int]:
        """Analyze chunks for multimodal content.
        
        Args:
            chunks: List of document chunks
            
        Returns:
            Analysis results with multimodal content counts
        """
        analysis = {
        ...
        for chunk in chunks:
            ...
            if re.search(r'^\|.*\|\s*$', chunk.text, re.MULTILINE) or 'csv' in chunk.text.lower():
                analysis["chunks_with_tables"] += 1
                has_multimodal = True
            
            # Check for figure patterns
            if re.search(r'!\[.*?\]\(.*?\.(png|jpg|jpeg|gif|svg)\)', chunk.text, re.IGNORECASE) or 'figure' in chunk.text.lower():
                analysis["chunks_with_figures"] += 1
                has_multimodal = True
            
            if has_multimodal:
                analysis["multimodal_chunk_count"] += 1
        
        return analysis
    
    def _extract_multimodal_references(self, content: str, source_filename: str) -> Tuple[str, Dict[str, List[str]]]:
        """Extract and catalog multimodal content references from markdown.
        ...
        Returns:
            Tuple of (content, multimodal_references)
        """
        multimodal_refs = {
            "tables": [],
            "figures": [],
        ...
        # Extract table references (markdown tables and CSV references)
        import re
        ...
        if table_matches:
            multimodal_refs["tables"].extend([f"inline_table_{i}" for i in range(len(table_matches))])
        
        # Find CSV/table file references
        csv_pattern = re.compile(r'!\[.*\]\(.*\.csv\)|\[.*\.csv\]', re.IGNORECASE)
        csv_matches = csv_pattern.findall(content)
        multimodal_refs["table_references"].extend(csv_matches)
        
        # Find image references
        img_pattern = re.compile(r'!\[.*?\]\(.*?\.(png|jpg|jpeg|gif|svg)\)', re.IGNORECASE)
        img_matches = img_pattern.findall(content)
        multimodal_refs["figure_references"].extend([match for match in img_matches])
        
        # Find figure file references
        fig_pattern = re.compile(r'!\[.*\]\(.*\.(png|jpg|jpeg|gif|svg)\)', re.IGNORECASE)
        fig_matches = fig_pattern.findall(content)
        multimodal_refs["figures"].extend([f"figure_{i}" for i in range(len(fig_matches))])
        
        return content, multimodal_refs
    
    def _find_orphaned_multimodal_references(self, chunks: List) -> List[str]:
        """Find multimodal references that may be orphaned (missing actual content).
        
        Args:
            chunks: List of document chunks
            
        Returns:
            List of chunk IDs with potentially orphaned multimodal references
        """
        orphaned = []
        ...
        for chunk in chunks:
            # Look for references to files that might not exist
            file_refs = re.findall(r'\[.*?\]\([^)]+\.(csv|png|jpg|jpeg|gif|svg)\)', chunk.text, re.IGNORECASE)
            
            # Check for broken image/table references
            broken_refs = re.findall(r'!\[.*?\]\(\s*\)', chunk.text)  # Empty image references
            
            if file_refs or broken_refs:
                orphaned.append(chunk.chunk_id)
        
        return orphaned
    
    async def generate_augmentoolkit_dataset(
        self, 
        chunks: List, 
        output_dir: str, 
        ...
        **config_kwargs
        ...
        Args:
            ...
            dataset_type: Type of dataset to generate (factual, rag, multi_source, repvar, complete)
            model_name: Model name or path for generation
            mode: Engine mode (mlx, api, local)
            max_chunks: Maximum chunks to process
            **config_kwargs: Additional configuration options
            
        Returns:
            Path to generated dataset or None if failed
        """
        if not HAS_AUGMENTOOLKIT:
            logger.warning("Augmentoolkit not available. Advanced dataset generation features disabled.")
            logger.info("To enable Augmentoolkit features, install with: pip install -e '.[augmentoolkit]'")
            return None
        
        try:
            logger.info(f"Generating {dataset_type} dataset using Augmentoolkit from {len(chunks)} chunks")
            
            # Import the enhanced generator function
            from .augmentoolkit_generator import generate_advanced_dataset
            
            # Convert chunks to the format expected by Augmentoolkit
            chunk_dicts = []
            for i, chunk in enumerate(chunks):
                chunk_dict = {
                    ...
                    "metadata": {
                        ...
                        "tokens": getattr(chunk, 'tokens', len(chunk.text.split()))
                    }
                }
                chunk_dicts.append(chunk_dict)
            
            # Save chunks to temporary directory for Augmentoolkit processing
            import tempfile
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i, chunk_dict in enumerate(chunk_dicts):
                    chunk_file = os.path.join(chunks_dir, f"chunk_{i:04d}.json")
                    with open(chunk_file, 'w', encoding='utf-8') as f:
                        json.dump(chunk_dict, f, indent=2, ensure_ascii=False)
                
                # Generate dataset using the advanced pipeline
                ...
                if success_count > 0:
                    logger.info(f"🎉 {success_count}/{total_count} datasets generated successfully")
                    return output_dir
                else:
            ...
            error_msg = f"Error generating Augmentoolkit dataset: {str(e)}"
            logger.error(error_msg)
            self.stats["errors"].append(error_msg)
            return None
    
    async def generate_complete_augmentoolkit_suite(
        self,
        chunks: List,
        output_dir: str,
        model_name: str = "mlx-community/Llama-3.2-3B-Instruct-4bit",
        ...
        """Generate a complete suite of Augmentoolkit datasets.
        
        This method creates all available dataset types: factual QA, RAG training,
        multi-source reasoning, and representation variation datasets.
        ...
        Returns:
            Dictionary mapping dataset types to their output paths
        """
        dataset_types = ["factual", "rag", "multi_source", "repvar"]
        results = {}
        
        logger.info(f"Generating complete Augmentoolkit dataset suite with {len(dataset_types)} types")
        
        for dataset_type in dataset_types:
            type_output_dir = os.path.join(output_dir, dataset_type)
            os.makedirs(type_output_dir, exist_ok=True)
            
            logger.info(f"Generating {dataset_type} dataset...")
            
            result_path = await self.generate_augmentoolkit_dataset(
                chunks=chunks,
            ...
            results[dataset_type] = result_path
            
            if result_path:
                logger.info(f"✅ {dataset_type} dataset completed: {result_path}")
            else:
                logger.error(f"❌ {dataset_type} dataset failed")
        ...
        summary = {
            ...
            "success_count": len([path for path in results.values() if path is not None]),
            "total_count": len(dataset_types)
        }
        
        summary_path = os.path.join(output_dir, "augmentoolkit_suite_summary.json")
        with open(summary_path, 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)
        
        logger.info(f"📋 Complete suite summary saved to: {summary_path}")
    ...
    def generate_summary(self, output_paths: Dict[str, Path]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "multimodal_processing": {
                ...
                "figures_extracted": self.stats["figures_extracted"],
                "multimodal_chunks_created": self.stats["multimodal_chunks"],
                "multimodal_files_processed": self.stats["multimodal_files_processed"],
                "multimodal_enabled": self.config.docling.multimodal,
                "extract_tables_enabled": self.config.docling.extract_tables,
                "extract_figures_enabled": self.config.docling.extract_figures
            },
    ...
    def run(
        ...
        try:
            ...
            # Phase 4: Concatenate Markdown files with multimodal tracking
            print(f"\n[PHASE 4] Document Concatenation")
            print(f"[PHASE 4] Concatenating {len(markdown_files)} Markdown files with multimodal tracking...")
            logger.info("Concatenating Markdown files with multimodal content tracking...")
            combined_path = str(output_paths["combined"] / "combined_document.md")
            combined_path, combined_multimodal_metadata = self.concatenate_markdown_files(markdown_files, combined_path)
            ...
            # Phase 6: Create RAG datasets with multimodal support
            print(f"\n[PHASE 6] RAG Dataset Creation")
            print(f"[PHASE 6] Creating RAG datasets from {len(chunks)} chunks with multimodal support...")
            logger.info("Creating RAG datasets with multimodal content tracking...")
            rag_dataset_path, finetune_dataset_path = self.create_rag_datasets(
                chunks, 
                str(output_paths["rag"]),
                multimodal_metadata=combined_multimodal_metadata
            )
            if rag_dataset_path:
            ...
            print(f"[PIPELINE] Multimodal content: {self.stats['tables_extracted']} tables, {self.stats['figures_extracted']} figures")
            print(f"[PIPELINE] Multimodal chunks: {self.stats['multimodal_chunks']}")
            print(f"[PIPELINE] Summary: {summary_path}")
            print(f"{'='*60}")
            logger.info(f"Pipeline completed successfully in {processing_time:.2f} seconds")
            logger.info(f"Multimodal processing: {self.stats['tables_extracted']} tables, {self.stats['figures_extracted']} figures extracted")
            logger.info(f"Multimodal chunks created: {self.stats['multimodal_chunks']}")
