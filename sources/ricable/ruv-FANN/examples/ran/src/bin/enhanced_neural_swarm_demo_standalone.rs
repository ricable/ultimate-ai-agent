...
// Simple pseudo-random number generator for standalone operation
struct SimpleRng {
    state: u64,
}
...
impl SimpleRng {
    ...
    fn gen_range(&mut self, min: f64, max: f64) -> f64 {
        let normalized = (self.next() % 1000000) as f64 / 1000000.0;
    ...
    for i in 1..=50 {
        ...
        for hour in 0..168 {
            ...
            let kpi = KpiMetrics {
                ...
                cell_load_percent: (load_factor * 60.0 + rng.gen_range(-10.0, 10.0)).max(5.0).min(95.0),
...
fn generate_realistic_throughput(cell_type: &str, load_factor: f64, rng: &mut SimpleRng) -> f64 {
    let base = if cell_type == "NR" { 300.0 } else { 150.0 };
    let noise = rng.gen_range(-20.0, 20.0);
    (base * load_factor + noise).max(10.0).min(500.0)
}

fn generate_realistic_latency(cell_type: &str, load_factor: f64, rng: &mut SimpleRng) -> f64 {
    let base = if cell_type == "NR" { 8.0 } else { 15.0 };
    let congestion_impact = load_factor * 10.0;
    let noise = rng.gen_range(-2.0, 2.0);
    (base + congestion_impact + noise).max(5.0).min(50.0)
}

fn generate_realistic_rsrp(cell_index: usize, rng: &mut SimpleRng) -> f64 {
    let base_distance_factor = (cell_index as f64 % 10.0) * -5.0;
    let noise = rng.gen_range(-10.0, 10.0);
    (-85.0 + base_distance_factor + noise).max(-140.0).min(-70.0)
}

fn generate_realistic_sinr(rng: &mut SimpleRng) -> f64 {
    let base = 12.0;
    let noise = rng.gen_range(-8.0, 8.0);
    (base + noise).max(-5.0).min(25.0)
}

fn generate_realistic_handover_rate(cell_type: &str, rng: &mut SimpleRng) -> f64 {
    let base = if cell_type == "NR" { 97.5 } else { 95.0 };
    let noise = rng.gen_range(-2.0, 2.0);
    (base + noise).max(85.0).min(99.5)
}

fn generate_realistic_energy(cell_type: &str, load_factor: f64, rng: &mut SimpleRng) -> f64 {
    let base = if cell_type == "NR" { 25.0 } else { 20.0 };
    let load_impact = load_factor * 15.0;
    let noise = rng.gen_range(-2.0, 2.0);
    (base + load_impact + noise).max(8.0).min(40.0)
}
