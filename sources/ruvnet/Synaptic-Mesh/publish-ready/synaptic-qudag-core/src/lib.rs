...
use ed25519_dalek::{Signature, Signer, PublicKey, SecretKey, Verifier};
...
pub mod crypto {
    ...
    /// Sign a QuDAG node
    pub fn sign_node(node: &mut QuDAGNode, secret_key: &SecretKey) -> Result<()> {
        let message = [
            node.id.as_bytes(),
            &node.data,
        ...
        let expanded_secret = ed25519_dalek::ExpandedSecretKey::from(secret_key);
        let signature = expanded_secret.sign(&message, &PublicKey::from(secret_key));
    ...
    pub fn verify_node_signature(node: &QuDAGNode, public_key: &PublicKey) -> Result<bool> {
...
mod tests {
    ...
    fn test_node_signing() {
        ...
        // Sign the node
        crypto::sign_node(&mut node, &secret_key).unwrap();
        assert!(node.signature.is_some());
        
        // Verify the signature
        let is_valid = crypto::verify_node_signature(&node, &public_key).unwrap();
        assert!(is_valid);
    }
    ...
    async fn test_consensus() {
        ...
        let node2_id = network.add_node(node2).await.unwrap();
        
        let node3 = QuDAGNode::new(b"node3");
        network.add_node(node3).await.unwrap();
        
        // Check confirmation
        assert!(consensus.is_confirmed(&node1_id));
        assert_eq!(consensus.confirmation_depth(&node1_id), 2);
    }
}